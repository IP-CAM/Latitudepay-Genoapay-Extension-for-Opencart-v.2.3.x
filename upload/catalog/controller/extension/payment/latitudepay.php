...
class ControllerExtensionPaymentLatitudePay extends Controller {
	...
	public function send() {
		...
		$payment_plan_message = 'LatitudePay Initiated. Payment Plan ID: '.$response->reference.'. Merchant Reference: '.$response->merchantReference.'. Transaction Token: '.$response->token.'. Customer Payment URL: '.$response->paymentUrl.'.';
		$this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get('latitudepay_entry_pending_status_id'), $payment_plan_message);
	...
	private function onlineSale($order_info){
		$this->model_extension_payment_latitudepay->log("REQUEST ONLINE SALE:");
