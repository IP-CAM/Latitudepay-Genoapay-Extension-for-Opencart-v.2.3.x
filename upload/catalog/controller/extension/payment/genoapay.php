	...
	public function success_webhook() {
		...
		if($this->verifyJsonApiResponse($url)){
			...
			if ($order) {
				...
				if ($order['order_status_id'] !== $this->config->get('genoapay_entry_success_status_id')){
				...
				$this->model_extension_payment_genoapay->log('Error occurred. Order does not exist. Please try again.');
				$this->response->redirect($this->url->link('checkout/checkout', '', true));
			}
		} else {
			...
			$this->model_extension_payment_genoapay->log('Signature mismatch. API Response has been tampered. Please investigate.');
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}
	}
	...
	public function callback_webhook() {
		...
		if($this->verifyJsonApiResponse($url)){
			...
			if ($order) {
				// if order hasn't been completed, add it to order history and make it complete
				if ($order['order_status_id'] !== $this->config->get('genoapay_entry_success_status_id')){
					$success_message = 'Payment of '.$this->currency->format($order['total'], $order['currency_code']).' was successful via Genoapay Callback. Transaction Token: '.$this->request->get['token'].'.';
					$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('genoapay_entry_success_status_id'),$success_message);
					$this->model_extension_payment_genoapay->updateGenoapayOrderStatus($order_id, 1);
				}
				return;
