...
class ControllerExtensionPaymentGenoapay extends Controller {
	public function index() {
		// does not need to be validated with NZD because getMethod from model makes the payment method disabled when not in AUD anyway
		...
		// enable genoapay checkout snippet
		$data['checkout_snippet'] = '<img src="https://images.latitudepayapps.com/v2/snippet.svg?amount='.$checkout_amount.'&services=GPAY&style=checkout" /><script src="https://images.latitudepayapps.com/v2/util.js"></script>';

		$data['genoapay_submit'] = $this->url->link('extension/payment/genoapay/send', '', true);
		return $this->load->view('extension/payment/genoapay', $data);
	}
    ...
    public function product_snippet(&$route, &$data, &$output) {
		...
		if (in_array(strtoupper($this->session->data['currency']), $currencies)) {
			...
			// onsite messaging
			$product_snippet = '<img style=\"max-width: 100%\" src=\"https://images.latitudepayapps.com/v2/snippet.svg?amount='.$product_amount.'&services=GPAY&style=default\" />';

			// prepend footer
			$data['footer'] = '
			<!-- Genoapay PDP Snippet Start -->
	...
	/**
	 * The function executed when checking out with Genoapay
	 */
	public function send() {
		...
		if($order_total < $this->config->get('genoapay_minimum_total')){
			$this->session->data['error'] = 'Minimum total for Genoapay is $'.$this->config->get('genoapay_minimum_total').'. Your order is only: $'.$order_total;
			$this->model_extension_payment_genoapay->log($this->session->data['error']);
		...
		if (isset($response->error)){
			$this->session->data['error'] = 'Error obtaining Authorisation Token from Genoapay: '.$response->error;
			$this->model_extension_payment_genoapay->log($this->session->data['error']);
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}

		if (!isset($response->authToken)){
			$this->session->data['error'] = 'Critical Error obtaining Authorisation Token from Genoapay.';
			$this->model_extension_payment_genoapay->log($this->session->data['error']);
		...
		// handle sale error
		if (isset($response->error)){
			$this->session->data['error'] = 'Error obtaining Transaction Token from Genoapay: '.$response->error;
			$this->model_extension_payment_genoapay->log($this->session->data['error']);
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}
		...
		if (!isset($response->token)){
			$this->session->data['error'] = 'Critical Error obtaining Transaction Token from Genoapay.';
			$this->model_extension_payment_genoapay->log($this->session->data['error']);
			$this->model_extension_payment_genoapay->log($response);
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}
	...
	public function fail_webhook() {
		...
		if(!$this->verifyJsonApiResponse($url)){
			...
			$this->model_extension_payment_genoapay->log($errorMessage);
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}
	...
	public function success_webhook() {
		$this->load->model('checkout/order');
		$this->load->model('extension/payment/genoapay');
		$this->model_extension_payment_genoapay->log('success_webhook');
		...
		if ($order['order_status_id'] === $this->config->get('genoapay_entry_pending_status_id')){
			$this->model_extension_payment_genoapay->log("Order $order_id changed from PENDING to SUCCESS.");
			$success_message = 'Payment of '.$this->currency->format($order['total'], $order['currency_code']).' was successful via Genoapay. Transaction Token: '.$this->request->get['token'].'.';
			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('genoapay_entry_success_status_id'),$success_message);
			$this->model_extension_payment_genoapay->updateGenoapayOrderStatus($order_id, 1);
		} else {
			$this->model_extension_payment_genoapay->log("Order $order_id is not PENDING, no action taken.");
		}
	...
	public function callback_webhook() {
		$this->load->model('checkout/order');
		$this->load->model('extension/payment/genoapay');
		$this->model_extension_payment_genoapay->log('callback_webhook');
		...
		if ($result == "COMPLETED"){
			// if order is PENDING, make it complete and add to order history
			if ($order['order_status_id'] === $this->config->get('genoapay_entry_pending_status_id')){
				$this->model_extension_payment_genoapay->log("Callback: Order $order_id changed from PENDING to SUCCESS.");
				$success_message = 'Callback: Payment of '.$this->currency->format($order['total'], $order['currency_code']).' was successful via Genoapay. Transaction Token: '.$this->request->get['token'].'.';
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('genoapay_entry_success_status_id'),$success_message);
				$this->model_extension_payment_genoapay->updateGenoapayOrderStatus($order_id, 1);
			} else {
				$this->model_extension_payment_genoapay->log("Callback: Order $order_id is not PENDING, no action taken.");
			}
		} else if ($result == "FAILED"){
			$this->model_extension_payment_genoapay->log("Callback: Order $order_id changed to FAILED.");
			$message = $_REQUEST['message'] ?? null;
			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('genoapay_entry_failed_status_id'), "Callback: $message");
			$this->model_extension_payment_genoapay->updateOrderStatus($order_id, $this->config->get('genoapay_entry_failed_status_id'));
		}
	...
	private function jsonBuilderForSale($order_info){
		...
		$shippingCost = $this->session->data['shipping_method']['cost'] ? $this->session->data['shipping_method']['cost'] : 0;
		...
		$jsonBody = '
		{
			...
			"returnUrls": {
				"successUrl": "'.$this->url->link('extension/payment/genoapay/success_webhook', '', true).'",
				"failUrl": "'.$this->url->link('extension/payment/genoapay/fail_webhook', '', true).'",
				"callbackUrl": "'.$this->url->link('extension/payment/genoapay/callback_webhook', '', true).'"
			}
		}
		';
		return $jsonBody;
	...
	private function verifyJsonApiResponse($url){
		...
		$api_secret	= $this->config->get('genoapay_environment') ? $this->config->get('genoapay_production_api_secret') : $this->config->get('genoapay_sandbox_api_secret');
	...
	private function getConfiguration(){
		...
		if (!isset($response->minimumAmount)){
			...
			// update database
			$this->model_extension_payment_genoapay->editSettingValue('genoapay', 'genoapay_minimum_total', $this->configurationMinimumAmount,  $this->config->get('config_store_id'));
			$this->model_extension_payment_genoapay->editSettingValue('genoapay', 'genoapay_configuration_last_update', $this->configurationLastUpdate,  $this->config->get('config_store_id'));
		}
	}
