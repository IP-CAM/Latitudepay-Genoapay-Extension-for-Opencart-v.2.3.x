...
class ModelExtensionPaymentGenoapay extends Model {
	...
	public function install() {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "genoapay_order` (
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "genoapay_refund_order` (
			`id` INT(11) NOT NULL AUTO_INCREMENT,
			`order_id` int(11) NOT NULL,
	...
	public function getOrder($order_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "genoapay_order WHERE `order_id` = '$order_id'");
        return $query->row;
    }
	...
	public function addGenoapayRefundOrder($order_id, $refund_amount, $refund_id, $refund_date, $reference, $commission_amount) {
        $sql = "
        	INSERT INTO ".DB_PREFIX."genoapay_refund_order(`order_id`, `refund_amount`, `refund_id`, `refund_date`, `reference`, `commission_amount`, `date_added`) VALUES('".$order_id."', '".$refund_amount."', '".$refund_id."', '".$refund_date."', '".$reference."', '".$commission_amount."', NOW());
        ";
	...
	public function getTotalRefunded($order_id) {
		$query = $this->db->query("SELECT SUM(`refund_amount`) AS `total` FROM `" . DB_PREFIX . "genoapay_refund_order` WHERE `order_id` = '" . (int)$order_id . "'");
